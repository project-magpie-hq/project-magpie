...
프로젝트 전체에서 단일 AsyncMongoClient 인스턴스를 재사용한다.
...
def get_client() -> AsyncMongoClient:
    """프로젝트 공용 AsyncMongoClient를 반환한다 (lazy singleton)."""
    global _client
    if _client is None:
        _client = AsyncMongoClient(_MONGO_URL)
    return _client


def get_db() -> AsyncDatabase:
    """프로젝트 공용 AsyncDatabase('the_nest')를 반환한다."""
