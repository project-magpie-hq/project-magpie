...
def build_target_refresh_inputs(user_id: str) -> dict:
    return {
        "user_id": user_id,
        "messages": [
            (
                "user",
                "EXPIRED 상태의 monitoring target이 있습니다. 현재 전략, 지갑, 기존 타점을 참고해 "
...
async def invoke_graph_for_target_refresh(refresh_graph, user_id: str) -> None:
    ...
    inputs = build_target_refresh_inputs(user_id)
