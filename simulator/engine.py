...
class TimeMachineSimulator:
    ...
    async def run_simulation(self):
        print("🚀 [Time Machine]: 시뮬레이션을 시작합니다...\n")
        ...
        await asyncio.gather(
            self._trigger_agents(start_time, reason="최초 타점 계산"),
        )

        print("✅ [초기화 완료]: 첫 타점이 계산되었습니다. 타임 루프를 시작합니다!\n")

        for current_time in self.timeline:
            if current_time == start_time:
                continue

            trade_occurred = self._check_virtual_execution(current_time)

            is_9am = current_time.hour == 9

            if is_9am or trade_occurred:
                await asyncio.gather(
    ...
    def _check_virtual_execution(self, current_time) -> bool:
        ...
        for coin, target in list(targets.items()):
            ...
            if coin not in self.state["stats"]:
                self.state["stats"][coin] = {"invested": 0, "returned": 0}
            ...
            if target["state"] == "WAITING_BUY" and target["hours_passed"] > target["valid_for_n_candles"]:
                ...
                self.state["trade_history"].append(f"[{current_time}] ⏳ {coin} 타점 만료")
                continue

            # 🔴 2. 매도 로직 (손절 최우선 검사)
            if target["state"] == "HOLDING" and self.state["holdings"].get(coin, 0) > 1:
                if self._handle_sell_logic(target, current_time, coin, candle):
                    any_trade_occurred = True

            # 🟢 3. 매수 로직 (매도 로직과 elif 관계 유지)
            elif target["state"] == "WAITING_BUY" and self._handle_buy_logic(target, current_time, coin, candle):
                any_trade_occurred = True

        return any_trade_occurred

    def _handle_sell_logic(self, target, current_time, coin, candle) -> bool:
        sell_price = None
        reason = ""
        ...
        if sell_price:
            ...
            self.state["trade_history"].append(
                f"[{current_time}] {reason} | {coin} | {holdings}개 매도 체결: {sell_price:,.0f}원"
            )
            return True
        return False

    def _handle_buy_logic(self, target, current_time, coin, candle) -> bool:
        buy_price = None
    ...
    async def _trigger_agents(self, current_time, reason: str):
        ...
        user_message = self._build_agent_user_message()
        initial_state = {
        ...
        config = {"configurable": {"thread_id": {"test_simulator_0001"}}}

        result_state = await self.magpie_graph.ainvoke(initial_state, config)
        self._parse_agent_results(result_state)

    def _build_agent_user_message(self) -> str:
        history_list = self.state["trade_history"]
        ...
        return f"""
                ...
                [시스템 지시]
                위 매매 이력을 철저히 분석하고, 시장 상황에 맞춰 필요하다면 투자 전략을 갱신하세요. 불필요하다고 판단되면 억지로 투자 전략을 갱신할 필요는 없습니다.
                그 후 하위 에이전트(Meerkat)에게 [매매 이력] 분석 결과와 주의사항을 담은 `meerkat_feedback` 을 작성하여 타점 갱신을 지시하세요.
                """

    def _parse_agent_results(self, result_state):
            ...
            if hasattr(msg, "tool_calls") and msg.tool_calls:
                for call in msg.tool_calls:
                    if call["name"] == "register_monitoring_targets_to_nest":
                        ...
                        print(
                            f"   🦉 [전략 수정 완료]: {self.state['strategy']['target_coins']} 종목에 대해 전략이 갱신되었습니다."
                        )
    ...
    def _print_report(self):
        ...
        for coin, stats in self.state["stats"].items():
            invested = stats["invested"]
            returned = stats["returned"]
            if invested == 0:
                continue

            if self.state["holdings"].get(coin, 0) > 0:
                last_price = self.data_map[coin].iloc[-1]["close"]
                holdings = self.state["holdings"].get(coin, 0)
                returned += last_price * holdings
